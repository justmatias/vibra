...
- [x] Add `fastapi` and `uvicorn` to dependencies
- [x] Add `poe api` command to `pyproject.toml` to start the API server
- [x] Wire up the existing DI containers (`InfrastructureContainer`, `ServicesContainer`) into the FastAPI lifespan context

---

## 2. Auth endpoints

- [x] `GET /api/auth/authorize`: Return Spotify OAuth URL for the frontend to redirect to
- [x] `POST /api/auth/callback`: Exchange Spotify `code` param for access token; return token to client
- [x] `POST /api/auth/refresh`: Refresh an expired access token
- [x] `POST /api/auth/logout`: Clear server-side cached token
- [x] `GET /api/auth/me`: Return current user profile (`SpotifyUser`)
...
- [x] Remove `vibra/ui/` directory
- [x] Remove `streamlit` and related UI packages from `pyproject.toml`
- [x] Remove `poe dev` command (replace with `poe api`)
